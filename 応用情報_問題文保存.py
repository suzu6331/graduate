...
GITHUB_TOKEN = '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'
...
GITHUB_BASE_DIR = 'AP'
GITHUB_IMAGE_DIR = os.path.join(GITHUB_BASE_DIR, 'image').replace("\\", "/")  # GitHubはスラッシュを使用
...
def fetch_questions(url, file_id, local_year_path, local_image_path):
    ...
    # 画像インデックスの初期化
    mondai_image_index = 0  # mondaiのdata-image-index
    sentaku_image_index = 0  # sentaku/kaitouのdata-image-index
    global_image_counter = 0  # 画像の保存用の一意なインデックス
        ...
        if isinstance(content, NavigableString):
            ...
            if content.name == 'div' and 'img_margin' in content.get('class', []):
                ...
                if img_tag and img_tag.get('src'):
                    ...
                    img_filename = f"{file_id}-{global_image_counter}.gif"  # .gifに統一
                    # 画像を保存
                    save_image(absolute_img_url, img_filename, local_image_path)
                    # imgタグの src を置き換える
                    mondai_html_parts.append(f'<div class="img_margin"><img data-image-index="{mondai_image_index}"></div>')
                    mondai_image_index += 1
    ...
    # sentaku_image_index をmondaiの画像数に基づいて設定
    sentaku_image_index = mondai_image_index if mondai_image_index > 0 else 0

    # 選択肢と回答
    sentaku = []
    kaitou = None

    # 画像の有無を確認するために、sentaku内の画像の数を事前に取得
    num_choices = len(ans_ele.find_all('li'))

    for li in ans_ele.find_all('li'):
        img_tag = li.find('img')
        if img_tag:
            img_url = img_tag['src']
            absolute_img_url = urljoin(url, img_url)
            ...
            if num_choices == 1:
                # Kaitouの場合
                kaitou = f"<img data-image-index=\"0\">"
            else:
                # Sentakuの場合
                sentaku.append(f"<img data-image-index=\"{sentaku_image_index}\">")
            sentaku_image_index += 1  # sentaku_image_indexをインクリメント
            global_image_counter += 1
        else:
...
def main():
    ##################################################################
    base_url = 'https://www.ap-siken.com/kakomon/05_aki/q'  # 新しいベースURL
